...
class ManifestFactory(object):
    ...
    def __init__(self, version="2.0", mdbase="", imgbase="", mddir="", lang="en"):
        ...
        try:
            self.whichid = subprocess.getoutput('which identify')
        except:
            # No IM or not unix
            self.whichid = ""
...
class BaseMetadataObject(object):
    ...
    def __setattr__(self, which, value):
        ...
        elif which[0] != "_" and not which in self._properties and not which in self._extra_properties and not which in list(self._structure_properties.keys()):
            self.maybe_warn("Setting non-standard field '%s' on resource of type '%s'" % (which, self._type))
        elif which[0] != '_' and not type(value) in [str, str, list, dict] and not which in self._integer_properties and \
    ...
    def test_object(self, data):
        ...
        if type(data) in [str, str]:
            ...
            for d in data:
                if type(d) in [str, str] and not data.startswith('http'):
                    return False
    ...
    def set_metadata(self, mdhash):
        ...
        if mdk == ['label', 'value']:
            ...
            k = mdhash['label']
            v = mdhash['value']
            if type(k) in [str, str] and self._factory.add_lang:
                k = self.langhash_to_jsonld({self._factory.default_lang : k})
            elif type(k) == dict:
                k = self.langhash_to_jsonld(k)
            if type(v) in [str, str] and self._factory.add_lang:
    ...
    def toJSON(self, top=False):
        ...
        for (p,sinfo) in list(self._structure_properties.items()):
            if p in d:
                if type(d[p]) == list:
                    newl = []
                    for s in d[p]:
        ...
        return OrderedDict(sorted(list(d.items()), key=lambda x: KEY_ORDER_HASH.get(x[0], 1000)))
...
class ContentResource(BaseMetadataObject):
    ...
    def make_selection(self, selector, summarize=False):
        ...
        if type(selector) == str:
            ...
            selector = OrderedDict(sorted(list(selector.items()), key=lambda x: KEY_ORDER_HASH.get(x[0], 1000)))
...
class Image(ContentResource):
    ...
    def set_hw_from_iiif(self):
        ...
        try:
            if self._factory.image_auth_token:
                req = urllib.request.Request(requrl, headers={'Authorization': self._factory.image_auth_token})
            else:
    ...
    def set_hw_from_file(self, fn):
        ...
        if cmd:
            # Try IM
            try:
                info = subprocess.getoutput(cmd + ' -ping -format "%h %w" ' + fn)
                (h, w) = info.split(" ")
                self.height = int(h)
                self.width = int(w)
                return
